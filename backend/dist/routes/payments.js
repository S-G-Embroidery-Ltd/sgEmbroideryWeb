...
const orderPayment_1 = require("../services/orderPayment");
...
router.post('/initiate', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const site = (0, corsOrigins_1.getPrimarySiteUrl)();
        const paystackData = {
            email,
            amount: amountMinor,
...
router.post('/verify', async (req, res) => {
    try {
        ...
        if (paymentData.status === 'success' && orderId) {
            const result = await (0, orderPayment_1.markOrderPaidFromPaystack)(String(orderId), {
